...
def card_read():
    # 建立数据库连接
    SQL_conn = pymysql.connect(host="localhost", port=3306, user=SQL_user, passwd=SQL_passwd, db=SQL_db)
    # 得到数据库操作游标
...
def card_record():
    ...
    SQL_cur = SQL_conn.cursor(cursor=pymysql.cursors.DictCursor)
    relist=[]
    resdata = SQL_cur.execute("select * from t_record order by time desc limit 0,50")
    for i in range(0,resdata):
        sobj=SQL_cur.fetchone()
        relist.append({"card":"%s"%sobj["card"], "dorm": "%s"%sobj["dorm"], "text": "%s"%sobj["text"],"time": "%s"%str(sobj["time"])})
